...
export function getDiscoveryCategories(categories = []) {
  ...
  const legacyAgriculture = list.find(
    (category) => category?.slug?.trim().toLowerCase() === "environment"
  );
  ...
  for (const category of list) {
    ...
    // Renamed Environment badges retain their palette; use it for the merged
    // Agriculture button too, while keeping Agriculture's label and icon.
    const palette = id === "agriculture" && legacyAgriculture?.colorHex
      ? legacyAgriculture
      : category;
    unique.set(id, {
      id,
