...
// SMTP is used whenever MAIL_HOST is configured; MAIL_MAILER=resend forces the Resend fallback.
function useSmtp() {
  if (String(process.env.MAIL_MAILER || "").toLowerCase() === "resend") return false
  return Boolean(process.env.MAIL_HOST)
}

function smtpSender() {
  const address = process.env.MAIL_FROM_ADDRESS || process.env.MAIL_USERNAME
  if (!address) return DEFAULT_FROM
  return process.env.MAIL_FROM_NAME ? `${process.env.MAIL_FROM_NAME} <${address}>` : address
}

// Shared sender: returns the same { data: { id }, error } shape for both providers.
async function sendEmail({ to, subject, html, headers }) {
  if (useSmtp()) {
    const info = await getSmtpTransport().sendMail({ from: smtpSender(), to, subject, html, headers })
    return { data: { id: info.messageId }, error: null }
  }
  return getResend().emails.send({ from: DEFAULT_FROM, to, subject, html, headers })
}
...
export async function sendVerificationCodeEmail({ to, code, expiresMinutes = 120, maskedEmail }) {
  return sendEmail({
    to,
    subject: "Your IMPACT R&D verification code",
    html: otpHtml({ code, expiresMinutes, maskedEmail }),
    headers: {
...
export async function sendDonationThankYouEmail({ to, projectTitle, amountPhp, signature, cluster }) {
  return sendEmail({
    ...
    html: donationThankYouHtml({ projectTitle, amountPhp, signature, explorerUrl: solanaExplorerUrl(signature, cluster) }),
    headers: {
...
export async function sendPasswordResetEmail({ to, resetUrl, maskedEmail }) {
  return sendEmail({
    to,
    subject: "Reset your IMPACT R&D password",
    html: resetPasswordHtml({ resetUrl, maskedEmail }),
